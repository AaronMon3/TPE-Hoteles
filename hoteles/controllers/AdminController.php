...
class AdminController {
    ...
    private function checkLogin() {
        if (!isset($_SESSION['admin'])) {
            header('Location: /admin/login');  // Cambia a URL semántica
            exit;
        }
    }
    ...
    public function login() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($usuario == 'webadmin' && $password == 'admin') {
                ...
                header('Location: ' . BASE_URL . '/admin/dashboard');  // URL semántica
                exit;
            } else {
    ...
    public function logout() {
        session_destroy();
        header('Location: ' . BASE_URL . '/admin/login');  // URL semántica
        exit;
    }
    ...
    public function formHabitacion() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($file['error'] === UPLOAD_ERR_OK && $file['size'] <= $maxSize) {
                    ...
                    if (in_array($mime, $allowed)) {
                        ...
                        $dest = $uploadsDir . '/' . $filename;
                            ...
                            $imagen_url = BASE_URL . '/view/public/uploads/' . $filename;
                            if ($previous_image && strpos($previous_image, BASE_URL . '/view/public/uploads/') === 0) {
                                $prevPath = __DIR__ . '/../' . str_replace(BASE_URL . '/', '', $previous_image);
            ...
            header('Location: ' . BASE_URL . '/admin/habitaciones');  
            exit;
        }
    ...
    public function listaTipos() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            header('Location: ' . BASE_URL . '/admin/tipos');
            exit;
        }
        ...
        $stmt->execute([$id]);
        header('Location: ' . BASE_URL . '/admin/tipos');  // URL semántica
        exit;
    }
}
?>