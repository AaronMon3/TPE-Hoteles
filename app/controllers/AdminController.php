...
class AdminController {
    ...
    private function checkLogin() {
        if (!isset($_SESSION['admin'])) {
            redirect('admin/login');  // Cambia a URL semántica
            exit;
        }
    }
    ...
    public function login() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($usuario == 'webadmin' && $password == 'admin') {
                ...
                redirect('admin/dashboard');  // URL semántica
                exit;
            } else {
    ...
    public function logout() {
        session_destroy();
        redirect('admin/login');  // URL semántica
        exit;
    }
                ...
                if ($file['error'] === UPLOAD_ERR_OK && $file['size'] <= $maxSize) {
                    ...
                    if (in_array($mime, $allowed)) {
                        ...
                        $dest = $uploadsDir . '/' . $filename;
                            ...
                            $imagen_url = BASE_URL . '/app/view/public/uploads/' . $filename;
                            if ($previous_image && strpos($previous_image, BASE_URL . '/app/view/public/uploads/') === 0) {
    ...
    public function listaTipos() {
        ...
        redirect('admin/tipos');  // URL semántica
        exit;
    }
}
?>
